...
int run(int *pc, int *bp, int *sp) {
  ...
  while (1) {
    ...
    else if (i == PRTF) { t = sp + pc[1]; a = printf((char *)t[-1], t[-2], t[-3], t[-4], t[-5], t[-6]); pc += 2; }
...
int main() {
  int *pc, *bp, *sp, poolsz;
  int *t, *loc, *fib, *power2;

  poolsz = 256 * 1024;
  if (!(e = malloc(poolsz))) return -1;
  if (!(sp = malloc(poolsz))) return -1;
  memset(e, 0, poolsz);

  // Fibonacci: int fib(int n) { if (n<=0) return 0; if (n==1) return 1; return fib(n-1)+fib(n-2); }
  fib = e;
  *e++ = ENT; *e++ = 0;
  ...
  *e++ = JSR; *e++ = (int)fib; *e++ = ADJ; *e++ = 1; *e++ = PSH;
  *e++ = LLA; *e++ = 2; *e++ = LI; *e++ = PSH;
  *e++ = IMM; *e++ = 2; *e++ = SUB; *e++ = PSH;
  *e++ = JSR; *e++ = (int)fib; *e++ = ADJ; *e++ = 1;
  *e++ = ADD; *e++ = LEV;

  // power2: int power2(int n) { if(n==0) return 1; return 2*power2(n-1); }
  power2 = e;
  *e++ = ENT; *e++ = 0;
  *e++ = LLA; *e++ = 2; *e++ = LI; *e++ = PSH;
  *e++ = IMM; *e++ = 0; *e++ = EQ;
  ...
  *e++ = LLA; *e++ = 2; *e++ = LI; *e++ = PSH;
  *e++ = IMM; *e++ = 1; *e++ = SUB; *e++ = PSH;
  *e++ = JSR; *e++ = (int)power2; *e++ = ADJ; *e++ = 1;
  *e++ = MUL; *e++ = LEV;

  // main: 呼叫 fib(7) 與 power2(3)
  pc = e;
  *e++ = ENT; *e++ = 0;
  *e++ = IMM; *e++ = (int)"f(7)=%d\n"; *e++ = PSH;
  *e++ = IMM; *e++ = 7; *e++ = PSH;
  ...
  *e++ = IMM; *e++ = 3; *e++ = PSH;
  *e++ = JSR; *e++ = (int)power2; *e++ = ADJ; *e++ = 1; *e++ = PSH;
  *e++ = PRTF; *e++ = ADJ; *e++ = 2;
